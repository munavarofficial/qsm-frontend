...
const backendUrl = import.meta.env.VITE_BACKEND_URL;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/api/principal/add-notice/`,
        {
          method: 'POST',
          body: form,
          headers: {
  ...
  const handleDelete = async (noticeId) => {
    try {
      const response = await fetch(
        `${backendUrl}/api/principal/delete-notice/${noticeId}/`,
        {
          method: 'DELETE',
          headers: {
            "X-CSRFToken": csrfToken,
...
{/* Alert Message */}
         {message && (
           <div className="mb-8 mx-auto max-w-2xl">
             <div className="bg-white border-l-4 border-blue-500 rounded-lg p-4 shadow-lg">
